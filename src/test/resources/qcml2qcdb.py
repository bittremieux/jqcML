...
XML_META_DATA_PARAMETER = 'metaDataParameter'
...
def createSQLiteTables(conn):
    # Create tables
    sql = 'CREATE TABLE IF NOT EXISTS qcml (QC_ID_PK INTEGER PRIMARY KEY ASC AUTOINCREMENT, qcml_file VARCHAR(45) NULL, version VARCHAR(45) NULL, xmlns VARCHAR(200) NULL);'
    conn.execute(sql)
    ...
    conn.execute(sql)
    sql = 'CREATE TABLE IF NOT EXISTS meta_data_parameter (MP_ID_PK INTEGER PRIMARY KEY ASC AUTOINCREMENT, id VARCHAR(45) UNIQUE NOT NULL, name VARCHAR(45) NOT NULL, value VARCHAR(45) NULL, accession VARCHAR(45) NULL, unit_name VARCHAR(45) NULL, unit_accession VARCHAR(45) NULL, cv_ref INTEGER NOT NULL, unit_cv_ref INTEGER NULL, QA_ID_FK INTEGER NOT NULL, FOREIGN KEY (unit_cv_ref ) REFERENCES cv (CV_ID_PK ), FOREIGN KEY (cv_ref ) REFERENCES cv (CV_ID_PK ), FOREIGN KEY (QA_ID_FK) REFERENCES quality_assessment (QA_ID_PK));'
    conn.execute(sql)
...
class CV: 
    ...
    def toSQLite(self, conn):
        ...
        if DEBUG:
            print "\t - cv_pk = " + str(self.sqliteId) + " id = " +  self.id
        return self.sqliteId

class QcML:
    def __init__(self, qcml_file = "", version = "", xmlns = ""):
    ...
    def toXml(self, file):
        ...
        file.write('<' + XML_QCML + ' version=' + self.version + ' xmlns=' + self.xmlns + '>\n')
    ...
    def fromSQLite(self, conn, id):
        cursor = conn.cursor()
        self.sqliteId = id
        cursor.execute("SELECT qcml_file,version,xmlns from qcml where QC_ID_PK=:id;", {"id":qcml_pk})
        row = cursor.fetchone()
        self.QcMLFile = row[0]
        self.version = row[1]
        self.xmlns = row[2]
    ...
    def toSQLite(self, conn):
        cursor = conn.cursor()
        cursor.execute("insert into qcml(qcml_file,version,xmlns) values (:file, :version, :xmlns);", {"file":self.QcMLFile, "version":self.version, "xmlns":self.xmlns})
        cursor.execute('SELECT last_insert_rowid()')
...
class QualityAssessment:
    ...
    def fromXml(self, xml):
        ...
        for meta_data_parameter in xml.getElementsByTagName(XML_META_DATA_PARAMETER):
            self.paramList += [MetaDataParameter(xml = meta_data_parameter)]
...
class AttachmentParam(CVParam):
    ...
    def fromSQLite(self, row, conn):
        ...
        if DEBUG:
            line = " - Got attachment_parameter: (" 
            for item in row:
                line += "'"
                if len(str(item))>100:
                    line += str(item)[:10] + "..." + str(item)[-10:]
                else:
                    line += str(item)
                line += "',"
            line = line[:-1] + ")"
...
class MetaDataParameter(CVParam):
    def __init__(self, dictionary = {}, xml =  None):
        CVParam.__init__(self, dictionary)
        if dictionary:
            self.fromDict(dictionary)
    ...
    def toXml(self, file):
        file.write('\t\t<' + XML_META_DATA_PARAMETER)
        CVParam.toXml(self, file)
        file.write('/>' + "\n")
    ...
    def fromSQLite(self, row, conn):
        CVParam.fromSQLite(self, row)
        if DEBUG:
            print " - Got meta_data_parameter: " + str(row)
    ...
    def toSQLite(self, conn, assessment_pk, cv_list):
        ...
        if not self.id:
            while not self.id:
                random_id = "ID_" + str(random.random())[2:]
                cursor.execute('SELECT * from meta_data_parameter where id=:id', {"id":random_id})
                if not cursor.fetchone():
                    self.id = random_id

        cursor.execute("insert into meta_data_parameter (id, name, value, accession, unit_name, unit_accession, cv_ref, unit_cv_ref, QA_ID_FK)  \
                        values (:id, :name, :value, :accession, :unit_name, :unit_accession, :cv_ref, :unit_cv_ref, :QA_ID_FK);", 
                        {"id":self.id, "name": self.name,"value": self.value,"accession": self.accession,
                         "unit_name": self.unitName,"unit_accession": self.unitAccession, 
                         "cv_ref":cv_ref_sqlite_id,"unit_cv_ref":unit_cv_ref_sqlite_id,"QA_ID_FK":assessment_pk})
        cursor.execute('SELECT last_insert_rowid()')
        self.sqliteId = cursor.fetchone()[0]
        if DEBUG:
            print "\t - mp_pk = " + str(self.sqliteId) + "\t " + str(self.id) 
        return self.sqliteId
